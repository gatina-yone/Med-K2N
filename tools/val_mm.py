...
def main(cfg):
    ...
    for case in cases:
        dataset_modals = cfg['DATASET'].get('MODALS', None)
        dataset_target = cfg['DATASET'].get('TARGET_MODAL', None)
        dataset = eval(cfg['DATASET']['NAME'])(cfg['DATASET']['ROOT'], 'val', transform, dataset_modals, case, target_modal=dataset_target)
        # --- test set
        # dataset = eval(cfg['DATASET']['NAME'])(cfg['DATASET']['ROOT'], 'test', transform, cfg['DATASET']['MODALS'], case)

    model = eval(cfg['MODEL']['NAME'])(cfg['MODEL']['BACKBONE'], dataset.n_classes, dataset_modals)
