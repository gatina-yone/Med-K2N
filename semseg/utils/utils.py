    ...
    try:
        if 'SLURM_PROCID' in os.environ and not 'RANK' in os.environ:
            ...
            if cuda_visible_devices:
                available_gpus = [int(x) for x in cuda_visible_devices.split(',')]
                if gpu >= len(available_gpus):
                    raise ValueError(f"LOCAL_RANK ({gpu}) 超出了可用GPU数量 ({len(available_gpus)})")
        else:
            print("未检测到分布式环境变量，使用单卡训练")
            return 0
        ...
        # 初始化进程组
        print(f'| distributed init (rank {rank}/{world_size}, gpu {gpu})', flush=True)
        ...
        print(f"分布式训练初始化失败: {str(e)}")
        print("环境变量状态:")
        for var in ['CUDA_VISIBLE_DEVICES', 'WORLD_SIZE', 'RANK', 'LOCAL_RANK', 'MASTER_ADDR', 'MASTER_PORT']:
