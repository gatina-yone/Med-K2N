...
class Affine:
    ...
    def __call__(self, img, label):
        return TF.affine(img, self.angle, self.translate, self.scale, self.shear, TF.InterpolationMode.BILINEAR, antialias=True), TF.affine(label, self.angle, self.translate, self.scale, self.shear, TF.InterpolationMode.NEAREST, self.seg_fill)
...
class RandomRotation:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if random.random() < self.p:
            img = TF.rotate(img, random_angle, TF.InterpolationMode.BILINEAR, antialias=True, expand=self.expand, fill=0)
            mask = TF.rotate(mask, random_angle, TF.InterpolationMode.NEAREST, expand=self.expand, fill=self.seg_fill)
        return img, mask
